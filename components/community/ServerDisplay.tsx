...
import { DiscordGuildProps } from "@/lib/definitions";

export default function ServerDisplay({ data }: DiscordGuildProps) {
	const { data: sessionData } = useSession();
	useGSAP(() => {
		gsap.set("#content > *, #sub-content *", { opacity: 0, x: -150, filter: "blur(16px)" });
		gsap.to("#content > *, #sub-content *", { opacity: 1, x: 0, duration: 1.2, stagger: 0.1, filter: "blur(0px)", ease: "power4.out" });
	}, []);
	...
	return (
		<Stack id="content" maxWidth={"lg"} mt={5} sx={{ mx: "auto" }}>
			...
			<Stack gap={3} direction={"column"} width={"fit-content"} sx={{ mx: "auto", alignItems: "center" }}>
				...
				{!sessionData?.isMemberOfGuild && sessionData?.user && <JoinServer />}
			</Stack>
